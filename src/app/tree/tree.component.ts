...
export class TreeComponent implements OnChanges {
  ...
  private setEdgeVisibility() {
    ...
    this.cy
      .nodes(
        '[status = "incomplete"], [status = "attempted"], [status = "hidden"], [status = "locked"]'
      )
      .outgoers("edge")
    ...
    this.cy
      .nodes('[status = "complete"]')
      .outgoers(
        'edge[type = "blocks"][target != "27"][target != "31"][target != "33"]'
      )
      .css({ visibility: "visible" });
  ...
  private checkSpecialCases() {
    const scenario21Complete =
      this.cy.nodes("#21").data("status") === "complete";
    ...
    if (!scenario21Complete) {
      if (this.cy.nodes("#35").data("status") === "complete") {
        if (
          this.cy.nodes("#27").data("status") === "attempted" ||
        ...
        if (
          this.cy.nodes("#31").data("status") === "attempted" ||
    ...
    if (!scenario24Complete || scenario42Complete) {
      if (this.cy.nodes("#34").data("status") === "complete") {
        if (
          this.cy.nodes("#33").data("status") === "attempted" ||
    ...
    if (!scenario23Complete && !scenario43Complete) {
      if (
        this.cy.nodes("#26").data("status") === "attempted" ||
